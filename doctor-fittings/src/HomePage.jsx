...
import insta1 from "./assets/instagram/doctor-fittings-1.png";
import insta2 from "./assets/instagram/doctor-fittings-2.png";
import insta3 from "./assets/instagram/doctor-fittings-3.png";
import insta4 from "./assets/instagram/doctor-fittings-4.png";
...
// ── Instagram Carousel ───────────────────────────────────────────────────────
const instaPosts = [
  { image: insta1, alt: "Doctor Fittings - Premium Window Hardware Rajkot" },
  { image: insta2, alt: "Doctor Fittings - Aluminium Fittings Gujarat" },
  { image: insta3, alt: "Doctor Fittings - Window Sliding Rollers" },
  { image: insta4, alt: "Doctor Fittings - Hardware Products Rajkot" },
];
...
function InstagramCarousel() {
  ...
  const total = instaPosts.length;

  const prev = () => setCurrent((i) => (i - 1 + total) % total);
  const next = () => setCurrent((i) => (i + 1) % total);

  const getVisible = () => {
    return [0, 1, 2].map((offset) => instaPosts[(current + offset) % total]);
  };
    ...
    <section className="bg-[#f4f6fb] px-8 md:px-12 py-20">
      ...
      <div className="flex flex-col items-center mb-10">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2.5 mb-3 no-underline group"
        >
          <span
          ...
          <span className="font-['Barlow_Condensed',sans-serif] font-black text-2xl md:text-3xl text-[#1e2a6e] group-hover:text-[#cc2366] transition-colors">
            Follow Us on Instagram
          </span>
        </a>
        <p className="text-[#5a6080] text-sm">
          @doctor_fittings — Stay updated with our latest products &amp; news
        </p>
      </div>

      {/* Desktop: 3-up grid with arrows */}
      <div className="max-w-5xl mx-auto relative">
        {/* Arrow left */}
        <button
          onClick={prev}
          aria-label="Previous"
          className="absolute -left-5 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md border border-[#e2e6f0] flex items-center justify-center hover:border-[#cc2366] hover:text-[#cc2366] transition-all text-[#1e2a6e] cursor-pointer"
        >
          <svg
            className="w-5 h-5"
            fill="none"
        ...
        {/* 3-up grid (desktop) / single (mobile) */}
        <div className="hidden sm:grid grid-cols-3 gap-5">
          {getVisible().map((post, idx) => (
            <a
              key={idx}
              href="https://www.instagram.com/doctor_fittings/"
              target="_blank"
            ...
            >
              ...
              <div
                className="absolute top-3 right-3 w-7 h-7 rounded-lg flex items-center justify-center text-white opacity-80"
                style={{
                  background:
                    "linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%)",
                }}
        ...
        <div className="sm:hidden">
          ...
          >
            <img
              src={instaPosts[current].image}
              alt={instaPosts[current].alt}
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end justify-center pb-5">
              <span className="flex items-center gap-1.5 text-white text-xs font-semibold">
                <IconInstagram />
                View on Instagram
              </span>
        ...
        <button
          onClick={next}
          aria-label="Next"
          className="absolute -right-5 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-md border border-[#e2e6f0] flex items-center justify-center hover:border-[#cc2366] hover:text-[#cc2366] transition-all text-[#1e2a6e] cursor-pointer"
        >
          <svg
            className="w-5 h-5"
            fill="none"
        ...
        {instaPosts.map((_, i) => (
          <button
            ...
            className={`rounded-full border-none cursor-pointer transition-all duration-300 ${
              i === current
                ? "w-6 h-2.5 bg-[#cc2366]"
                : "w-2.5 h-2.5 bg-[#cc2366]/30 hover:bg-[#cc2366]/60"
            }`}
          />
        ))}
      </div>
      ...
      <div className="text-center mt-7">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-sm font-semibold no-underline px-6 py-2.5 rounded-full border border-[#cc2366]/40 text-[#cc2366] hover:bg-[#cc2366] hover:text-white transition-all duration-200"
        >
          <IconInstagram />
